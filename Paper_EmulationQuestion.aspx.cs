...
public partial class AuthoringTool_CaseEditor_Paper_Paper_EmulationQuestion : AuthoringTool_BasicForm_BasicForm
{
    ...
    protected void btnTrBack_Click(object sender, EventArgs e)
    {
        HiddenFieldTrStatusValue.Value = (Convert.ToInt32(HiddenFieldTrStatusValue.Value.ToString()) - 1).ToString();
        Response.Write(HiddenFieldTrStatusValue.Value.ToString());
    }

    protected void btnTrNext_Click(object sender, EventArgs e)
    {
        if (strSenceID == "")
            strSenceID = Session["SceneID"].ToString();

        if (HiddenFieldTrStatusValue.Value.ToString() == "0")
        {
            //判斷是否有設定VRversion
            if (strSenceID != "")
            {
    ...
    //控制目前應該顯示的欄位
    private void showTrFunc()
    {
        string strCaseSwitch = HiddenFieldTrStatusValue.Value.ToString();
        switch (strCaseSwitch)
        {
            case "0":
                trAnswerType.Visible = false;
                trSetAns.Visible = false;
                trSetLearningPoint.Visible = false;
                trsetTextBook.Visible = false;
                trSetSensitivity.Visible = false;
                trMode.Visible = false;

                btnFinish.Visible = false;
                btnTrNext.Visible = true;
                break;
            case "1":
            ...
            case "3":
                trAnswerType.Visible = true;
                trSetAns.Visible = true;
                trSetLearningPoint.Visible = true;
                trsetTextBook.Visible = false;
                trSetSensitivity.Visible = false;
                trMode.Visible = false;
    ...
    protected void rb_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (rb.ID.Equals("rbRecordStep"))
        {
            HiddenFieldTrStatusValue.Value = "1";
            showTrFunc();
        }
        else
        {
            HiddenFieldTrStatusValue.Value = "11";
            showTrFunc();
        }
    }
    ...
    protected void UpdateMode(string value)
    {

        string strSQL = "UPDATE VRTeachingAssistant  SET Mode = '" + value + "' WHERE strQuestionID = '" + strQID + "';";
        try
        {
            hintsDB.ExecuteNonQuery(strSQL);
        }
