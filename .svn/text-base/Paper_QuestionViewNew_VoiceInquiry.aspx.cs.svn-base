...
namespace PaperSystem
{
    ...
    /// </summary>
    public partial class Paper_QuestionViewNew_VoiceInquiry : AuthoringTool_BasicForm_BasicForm
    {
        //建立SqlDB物件
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            Ajax.Utility.RegisterTypeForAjax(typeof(Paper_QuestionViewNew_VoiceInquiry));
            ...
            if (!IsPostBack)
            {
                ...
                {
                    string strCareer = Request.QueryString["Career"]; //傳職業ID給下個頁面  老詹 2014/08/26
                    Response.Redirect("Paper_ConversationQuestionEditor.aspx?Opener=QGroupTreeNew&GroupID=" + strGroupID + "&Career=" + strCareer + "&bModify=False");
                }

                if (Session["ProblemTypeConfirm"] == null)
                {
                    hfSymptoms.Value = "All"; // =All即為一開始進頁面時顯示  老詹 2014/09/24
                }
                else
                {
                    string strGetProblemTypeConfirm = Session["ProblemTypeConfirm"].ToString();
                    InitiateProblemType = strGetProblemTypeConfirm.Split('/');
                    if (InitiateProblemType[1] != "Select a Child")  //若沒有Child時，則將前半段給hfSymptoms
                        hfSymptoms.Value = InitiateProblemType[1].ToString();
                    else
                        hfSymptoms.Value = InitiateProblemType[0].ToString();
                }
                ddlCareer.Items.Clear();
                //建立職業的下拉選單項目 
                DataTable dtCareerName = new DataTable();
                dtCareerName = BuildCareer();
                ddlCareer.Items.Add(new ListItem(dtCareerName.Rows[0]["cNodeName"].ToString(), dtCareerName.Rows[0]["cNodeID"].ToString()));
                Session["tmpCareer"] = ddlCareer.SelectedValue.ToString();
                ProblemType_SELECT();
                ...
                // 給定選單預設值
                ddlProblemTypeChild.Items.Add(new ListItem("Select a Child", "Select a Child"));

                //是否顯示Edit VP's Answer Set按鈕   老詹 2014/11/11
                string strDisplayBQL = Request.QueryString["bDisplayBQL"].ToString();
                if (strDisplayBQL == " Yes")
                {
                    btEditAnswerType.Visible = false;
                    //btEditVPAnswerSet.Visible = true;  // 註解掉，在編輯考卷時才能編輯VP's Answer  老詹 2015/04/07
                    //btEditVPAnswerSet.Attributes.Add("onclick", "OpenNewPage('" + ddlProblemTypeChild.SelectedItem.Text.ToString() + "')");
                }
                else
                {
                    btEditAnswerType.Visible = true;
                    btEditVPAnswerSet.Visible = false;
                }

              // 顯示已選好的ProblemType選單值   老詹 2014/08/27
              if (Session["ProblemTypeConfirm"] != null)
              {
                string strGetProblemTypeConfirm = Session["ProblemTypeConfirm"].ToString();
                InitiateProblemType = strGetProblemTypeConfirm.Split('/');
                ddlProblemType.Items.Clear();
                ddlProblemTypeChild.Items.Clear();
                ddlProblemType.Items.Add(new ListItem(InitiateProblemType[0].ToString(), InitiateProblemType[0].ToString()));
                ddlProblemTypeChild.Items.Add(new ListItem(InitiateProblemType[1].ToString(), InitiateProblemType[1].ToString()));
                ddlProblemType.Enabled = false;
                ddlProblemTypeChild.Enabled = false;
                btnConfirmProblemType.Visible = false;
                btnReselectProblemType.Visible = true;
                btEditVPAnswerSet.Attributes.Add("onclick", "OpenNewPage('" + InitiateProblemType[1].ToString() + "')");
              }
            }
        ...
        protected void setupConversationQuestionTable()
        {
            ...
            if (dsQuestionList.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < dsQuestionList.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (hfSymptoms.Value == "All" || hfSymptoms.Value == strSymptoms)
                    {
                        ...
                        tcKeyword.Text = tcKeyword.Text.Replace(",",", ");
                        tcKeyword.Text = tcKeyword.Text.Replace("|", " | ");
                        ...
                        //建立ProType的標題
                        TableRow trProTypeTitle = new TableRow();
                        trProTypeTitle.Attributes.Add("Class", "header1_tr_odd_row");
                        table.Rows.Add(trProTypeTitle);
                        trProTypeTitle.ID = "trProTypeTitle_" + strQID;

                        TableCell tcProTypeTitle = new TableCell();
                        trProTypeTitle.Cells.Add(tcProTypeTitle);
                        tcProTypeTitle.Text = "<font style='color:Black; font-weight:bold'>Problem Type :&nbsp; <font/>";
                        tcProTypeTitle.Style.Add("text-align", "right");
                        tcProTypeTitle.Width = Unit.Pixel(230);

                        TableCell tcProType = new TableCell();
                        trProTypeTitle.Cells.Add(tcProType);
                        string strGetParentNameSQL = "SELECT cNodeName FROM QuestionGroupTree WHERE cNodeID=(SELECT cParentID FROM QuestionGroupTree WHERE cNodeID='"+ strGroupID +"')";
                        DataTable dtGetParentName = HintsDB.getDataSet(strGetParentNameSQL).Tables[0];
                        tcProType.Text = dtGetParentName.Rows[0]["cNodeName"].ToString() + "/" + strGroupName;
                        tcProType.Style.Add("color", "Green");
                        tcProType.Attributes.Add("Class", "header1_tr_even_row");
                        ...
                        tcConversationQuestionTitle.Attributes.Add("onclick", "ShowConversationDetail('" + strQID + "','img_" + strQID + "','" + hfAnswerTypeCount.Value + "')");
        ...
        protected void btnModifyConversation_Click(object sender, EventArgs e)
        {
            //修改一個對話題   
            ...
            string strbDisplayBQL = Request.QueryString["bDisplayBQL"];
            if (strbDisplayBQL == " No")
                Response.Redirect("Paper_ConversationQuestionEditor.aspx?Opener=Paper_QuestionViewNew_VoiceInquiry&QID=" + strQID + "&AnswerType=" + strAnswerTypeNum[2] + "&GroupID=" + Session["GroupID"].ToString() + "&Career=" + strCareer + "&bModify=True");
            else
                Response.Redirect("Paper_ConversationQuestionEditor.aspx?Opener=Paper_QuestionViewNew_VoiceInquiry&QID=" + strQID + "&AnswerType=" + "1" + "&GroupID=" + Session["GroupID"].ToString() + "&Career=" + strCareer + "&bModify=True");
        }
        ...
        protected void ddlProblemType_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlProblemType.SelectedItem.Text == "Select a Problem Type")
                ...
                hfSymptoms.Value = ddlProblemType.SelectedItem.Text;

            intQuestionIndex = 0;

            ddlProblemTypeChild.Items.Clear();
            ProblemTypeChild_SELECT();
            //建立選擇題表格
            this.setupQuestionTable();
            //建立問答題表格
            this.setupTextQuestionTable();
            //建立對話題表格
            this.setupConversationQuestionTable();
            Session["tmpProblemType"] = ddlProblemType.SelectedValue.ToString();
        }

        protected void ddlProblemTypeChild_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlProblemTypeChild.SelectedItem.Text != "Select a Child")
                hfSymptoms.Value = ddlProblemTypeChild.SelectedItem.Text;
            else
                hfSymptoms.Value = ddlProblemType.SelectedItem.Text;
            //建立選擇題表格
            this.setupQuestionTable();
            ...
            //建立對話題表格
            this.setupConversationQuestionTable();
            Session["tmpProblemTypeChild"] = ddlProblemTypeChild.SelectedValue.ToString();
        }

        //取得Career項目(修改為前頁選取好的單一職業 2014/08/19)
        private DataTable BuildCareer()
        {
            clsHintsDB HintsDB = new clsHintsDB();
            DataTable dtCareer = new DataTable();
            string strCareer = Request.QueryString["Career"].ToString();
            string strSQL_Career = "SELECT DISTINCT cNodeID,cNodeName FROM ProblemTypeTree WHERE cNodeID = '" + strCareer + "' ORDER BY cNodeName ASC";
            dtCareer = HintsDB.getDataSet(strSQL_Career).Tables[0];
            return dtCareer;
        }
        //取得Problem Type項目  老詹 2014/08/14
        protected void ProblemType_SELECT()
        {
            ddlProblemType.Items.Add(new ListItem("Select a Problem Type", "Select a Problem Type"));
            clsHintsDB HintsDB = new clsHintsDB();
            DataTable dtTmp = BuildCareer();
            foreach (DataRow drTmp in dtTmp.Rows)
            {
                string strSQL_ProblemTypeTree = "SELECT DISTINCT cNodeName, cNodeID FROM ProblemTypeTree WHERE cParentID = '" + drTmp["cNodeID"].ToString() + "' ORDER BY cNodeName ASC";
                DataTable dtProblemTypeTree = HintsDB.getDataSet(strSQL_ProblemTypeTree).Tables[0];
                foreach (DataRow drProblemTypeTree in dtProblemTypeTree.Rows)
                {
                    ddlProblemType.Items.Add(new ListItem(drProblemTypeTree["cNodeName"].ToString(), drProblemTypeTree["cNodeID"].ToString()));
                    ddlProblemType.SelectedIndex = 0;
                }
            }
        }
        //取得Problem Type Child項目  老詹 2014/08/14
        protected void ProblemTypeChild_SELECT()
        {
            ddlProblemTypeChild.Items.Add(new ListItem("Select a Child", "Select a Child"));
            clsHintsDB HintsDB = new clsHintsDB();
            DataTable dtProblemTypeChild = new DataTable();
            string strSQL_ProblemTypeChild = "SELECT DISTINCT cNodeName, cNodeID FROM ProblemTypeTree WHERE cParentID = '" + ddlProblemType.SelectedValue + "' AND cCareer = '" + ddlCareer.SelectedItem.Text.ToString() + "' ORDER BY cNodeName ASC";
            dtProblemTypeChild = HintsDB.getDataSet(strSQL_ProblemTypeChild).Tables[0];
            foreach (DataRow drProblemTypeChild in dtProblemTypeChild.Rows)
            {
                ddlProblemTypeChild.Items.Add(new ListItem(drProblemTypeChild["cNodeName"].ToString(), drProblemTypeChild["cNodeID"].ToString()));
            }
        }

        //返回時仍能依職業建置樹   老詹 2014/08/19
        protected void GobackForCareer(object sender, EventArgs e)
        {
            Session["ProblemTypeConfirm"] = null;
            string strCareer = Request.QueryString["Career"];
            Response.Redirect("QuestionGroupTree/QGroupTreeNew.aspx?SelectedGroup="+ strGroupID +"&Career="+ strCareer);
        }
        protected void GonextForCareer(object sender, EventArgs e)
        {
            Session["ProblemTypeConfirm"] = null;
            string strCareer = Request.QueryString["Career"];
            Response.Redirect("Paper_QuestionMainNew.aspx?Career=" + strCareer);
        }
        ...
        protected void btAddQuestion_Click(object sender, EventArgs e)
        {
            strGroupID = Session["GroupID"].ToString();
            string strCareer = Request.QueryString["Career"]; //傳職業ID給下個頁面  老詹 2014/08/26
            Response.Redirect("Paper_ConversationQuestionEditor.aspx?Opener=Paper_QuestionViewNew_VoiceInquiry&GroupID=" + strGroupID + "&Career=" + strCareer + "&bModify=False");
        }
        protected void btEditAnswerType_Click(object sender, EventArgs e)
        {
            strGroupID = Session["GroupID"].ToString();
            string strCareer = Request.QueryString["Career"];
            Response.Redirect("Paper_AnswerTypeEdit.aspx?GroupID=" + strGroupID + "&Career=" + strCareer);
        }
        ...
        protected void btnConfirmProblemType_Click(object sender, EventArgs e)
        {
            ddlProblemType.Enabled = false;
            ddlProblemTypeChild.Enabled = false;
            Session["ProblemTypeConfirm"] = ddlProblemType.SelectedItem.Text.ToString() + "/" + ddlProblemTypeChild.SelectedItem.Text.ToString();
            btnReselectProblemType.Visible = true;
            btnConfirmProblemType.Visible = false;
            // 增加btEditVPAnswerSet的事件  老詹 2014/12/06
            btEditVPAnswerSet.Attributes.Add("onclick", "OpenNewPage('" + ddlProblemTypeChild.SelectedItem.Text.ToString() + "')");
        }
        protected void btnReselectProblemType_Click(object sender, EventArgs e)
        {
            btnConfirmProblemType.Visible = true;
            btnReselectProblemType.Visible = false;
            ddlProblemType.Enabled = true;
            ddlProblemTypeChild.Enabled = true;
            ddlProblemType.Items.Clear();
            ddlProblemTypeChild.Items.Clear();
            ProblemType_SELECT();
            ddlProblemTypeChild.Items.Add(new ListItem("Select a Child", "Select a Child"));
