...
//		nodeSelColor : "#FFFFCC",
        nodeSelColor : "#DCDCDC",  //FFFF99
//		levelColors : ["#5555FF","#8888FF","#AAAAFF","#CCCCFF"],
//		levelBorderColors : ["#5555FF","#8888FF","#AAAAFF","#CCCCFF"],
//		levelColors : ["#CCCCFF","#AAAAFF","#8888FF","#5555FF"],
//		levelBorderColors : ["#CCCCFF","#AAAAFF","#8888FF","#5555FF"],
			...
			switch (this.render)
			{
				...
				case "VML":
					...
					if (node.target && this.config.useTarget)
					{
						...
						s.push(node.dsc + "<br/>111");			
						s.push('</a>');	
					}				
					else
					{						
						var ResultCondition = node.dsc.split("$");
						if(ResultCondition.length>1)
						{
						 if(ResultCondition[1]!="")
						     s.push("Condition:" + ResultCondition[1] + "<br/><hr />Result:" + ResultCondition[0]);
						 else
						     s.push("Item:" + ResultCondition[0]);
						}
						else
						{
						     s.push("Item:" + ResultCondition[0]);
