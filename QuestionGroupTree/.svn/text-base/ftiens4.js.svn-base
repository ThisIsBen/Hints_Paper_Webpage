...
{ 
  ...
  if (USEICONS)
  {
    ...
    docW = docW + "<a><img nodeID='"+this.NodeID+"' ondrop='fnHandleDrop()' ondragover='fnCancelDefault()' ondragenter='fnHandleDragEnter()' ondragstart='fnHandleDragStart()' id='folderIcon" + this.id + "' name='folderIcon" + this.id + "' src='" + this.iconImageSrc() + "' border=0></a>"
  }
  else
  {
	  if (this.prependHTML == "")
...
{ 
  ...
  if (this.level>0) 
    ...
    { 
      leftSide = leftSide + "<td valign=top><img src='" + ICONPATH + "ftv2lastnode.gif' width=16 height=22 border=0></td>"
    } 
    else 
    { 
      leftSide = leftSide + "<td valign=top background='" + ICONPATH + "ftv2vertline.gif'><img src='" + ICONPATH + "ftv2node.gif' width=16 height=22 border=0></td>"
    } 
  ...
  docW = docW + "<tr>" + leftSide + "<td valign=top>"
  if (USEICONS)
      docW = docW + "<a>" + "<img id='itemIcon"+this.id+"' nodeID='"+this.NodeID+"' ondrop='fnHandleDrop()' ondragover='fnCancelDefault()' ondragenter='fnHandleDragEnter()' ondragstart='fnHandleDragStart()' " + "src='"+this.iconSrc+"' border=0>" + "</a>"
  else
	  if (this.prependHTML == "")
        docW = docW + "<img src='" + ICONPATH + "ftv2blank.gif' height=2 width=3 border=0>"
  ...
  {
     var tmpDesc = this.desc;
     //tmpDesc = "";     
     /*
     while(tmpDesc.indexOf('"')>=0)
     {
...
//NodeID與ID的意義不同,NodeID是Hints中用來識別的ID,而ID是此TreeView Control原作者的定義的
function getIDByNodeID(str_NodeID)
{
    var indexOfEntries_length = indexOfEntries.length;
    for(var i=0;i<indexOfEntries_length;i++)
    {
        if(indexOfEntries[i].NodeID==str_NodeID)
        {
            return i;
        }
    }
...
function leftSideHTML(leftSideCoded) {
	...
	for (i=0; i<leftSideCoded.length; i++)
	{
		...
		{
			retStr = retStr + "<td valign=top background='" + ICONPATH + "ftv2vertline.gif'><img border=0 src='" + ICONPATH + "ftv2vertline.gif' width=16 height=22></td>"
		}
		if (leftSideCoded.charAt(i) == "0")
		{
			retStr = retStr + "<td valign=top><img src='" + ICONPATH + "ftv2blank.gif' width=16 border=0 height=22></td>"
		}
	}
	return retStr
}
...
function insFld(nodeID,parentFolder, childFolder) 
{ 
  var nodeFld = parentFolder.addChild(childFolder);
  nodeFld.NodeID = nodeID;
  return nodeFld;
} 
 
function insDoc(nodeID,parentFolder, document) 
{ 
  var nodeDoc = parentFolder.addChild(document);
  nodeDoc.NodeID = nodeID;
  return nodeDoc;
} 
